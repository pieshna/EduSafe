...
export async function buscarPersonaPorRol(rol) {
  ...
  if (rol === 0) {
    const [rows] = await connection.query("SELECT * FROM usuarios WHERE estado=1 group by rol");
    return rows;
  } else {
    const [rows] = await connection.query("SELECT * FROM usuarios WHERE rol=? AND estado=1", [rol]);
    return rows;
  }
}
...
export async function crearPersona(req, res) {
  ...
  const validarCorreo = await connection.query("SELECT * FROM usuarios where correo=?", [
    req.body.correo,
  ]);
  ...
  const validarUsuario = await connection.query("SELECT * FROM usuarios WHERE usuario=?", [
    req.body.usuario,
  ]);
  ...
  res.json({
    Mensaje: `${req.body.nombre} ${req.body.apellido} ingresado exitosamente`,
    id: result,
  });
  return result;
}
...
export async function actualizarEstado(req, res, props) {
  const connection = await connect();
  if (props === 0) {
    const result = await connection.query("UPDATE usuarios SET estado=0 where id=?", [
      req.params.id,
    ]);
    res.json(result);
  } else {
    const result = await connection.query("UPDATE usuarios SET estado=1 where id=?", [
      req.params.id,
    ]);
    res.json(result);
  }
...
export async function actualizarPersona(req, res) {
  ...
  //valiamos que el correo y el usuario sean unicos
  /* const validarCorreo = await connection.query("SELECT * FROM usuarios where correo=? and id!=?", [
    req.body.correo,
    req.params.id,
  ]);
  let [rows] = validarCorreo;
...
export async function contadorPersonas(rol) {
  ...
  const [rows] = await db.query(
    "SELECT COUNT(id) as Actuales FROM usuarios WHERE rol=? AND estado=1",
    [rol]
  );
  return rows;
}
